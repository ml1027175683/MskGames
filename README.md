...
后端代码位于 `server/`，采用接近 Java MVC 的分层结构。
...
```text
controller/    # HTTP Controller，负责请求和响应
service/       # 业务逻辑层
repository/    # DB 访问层
model/         # 数据模型
router/        # 路由注册
...
| `npm.cmd run dev` | 启动前端本地开发服务器 |
| `npm.cmd test` | 运行前端测试 |
| `npm.cmd run test:watch` | 以监听模式运行前端测试 |
| `npm.cmd run build` | 执行 TypeScript 检查并构建前端生产版本 |
| `npm.cmd run preview` | 本地预览前端生产构建 |
