...
当前仓库已经具备一个可运行、可测试的网页 MVP 原型，重点是先验证基础玩法循环和前端交互：自动挖矿、颜色库存、画作库存、独立画布、自动保存和作品继续创作。

当前版本还不是完整经济系统。作品鉴定、唯一资产生成、平台内市场、Steam 接入仍属于后续路线。
...
| 画作库存 | 内置哥布林和皮卡丘像素作品，每张作品都可继续创作 |
| 画布 | 独立 `16x16` 像素画布，共 256 个像素 |
| 填色 | 选择已拥有颜色后点击像素填色，并消耗库存数量 |
...
当前原型已经覆盖前三步的主要交互。鉴定、唯一性校验、资产生成、市场和榜单仍在后续实现范围。
...
| 鉴定 | 未实现 | 消耗颜色和鉴定费，把作品转成唯一资产 |
| 唯一性 | 领域函数预留 | `画布尺寸 + 完整 RGB 像素矩阵` 全平台唯一 |
| 资产 | 未实现 | 鉴定成功后生成平台内资产记录 |
...
3. 库存页切换到画作库存后，可查看哥布林和皮卡丘像素作品。
4. 点击任意画作的继续创作，会打开独立 `16x16` 画布。
5. 在画布页选择一个可用色块，再点击像素即可填色。
6. 每次填色会自动保存，不需要点击保存按钮。
7. 画布缩放默认 `100%`，可在 `50%` 到 `3200%` 之间调整。
...
1. 稳定多画作状态管理，避免当前编辑作品和库存列表耦合。
2. 完善作品详情选中态，让画作库存详情可查看任意作品。
3. 完善 RGB 颜色消耗、库存不足和重复填色规则。
4. 实现作品鉴定、唯一性指纹和资产生成。
5. 实现平台内资产库、挂售和购买流程。
6. 增加评分、榜单和基础反刷规则。
7. 在资产模型稳定后补充 Steam Inventory / Steam Market 映射字段。
